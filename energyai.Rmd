...
```{r}
logit<-glm(Appliances ~., data = energydatam)
summary(logit)
...
## Decision Tree Model
```{r}
energydatadt <- as.data.frame(model.matrix(~.-1,energydata))
energydatadt$Appliances <- as.factor(energydatadt$Appliances)
decisiontreemodel <- C5.0(Appliances ~., data = energydatadt)
...
```{r}
energydecisionpredict <- predict(decisiontreemodel, energydatadt)
confusionMatrix(energydecisionpredict, energydatadt$Appliances)
```
...
testset <- sample(1:nrow(energydatam), 3947)

energytest <- energydatam[testset,]
energytrain <- energydatam[-testset,]

annmodel <- neuralnet(Appliances ~ ., data=energytrain)
```

### ANN Model Confusion Matrix
```{r}
neuralpredictions <- predict(annmodel, energytest)
neuralpredictions

#CrossTable(x= energydatam$Appliances, y= neuralpredictions, #prop.chisq=FALSE)
#confusionMatrix(as.factor(neuralpredictions), as.factor(energytest$Appliances), positive="1")
```
