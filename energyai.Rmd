...
annmodel <- neuralnet(Appliances ~ ., data=energytrain, hidden = 2)
plot(annmodel)
```
...
model_results <- compute(annmodel, energytest[2:25])
predicted_Appliance <- model_results$net.result
cor(predicted_Appliance, energytest$Appliances)


CrossTable(x= energytest$Appliances, y= neuralpredictions, prop.chisq=FALSE)
